...
class PaymentController extends Controller
{
    ...
    public function setCrypto(Request $request){
       ...
       if($request->token == 'BTC'){
        ...
        $payorder->order_id = $request->order;
        ...
        $payorder->transaction_time = (new \DateTime())->format('Y-m-d H:i:s');
        $payorder->save();
       } else if ($request->token == 'ETH'){
            $payorder = Payorders::where('order_id', $request->order)->first();
            if($payorder) return response()->json('failed', 201);;
            $payorder = new Payorders();
            // dump($payorder);
            // dd($response);
            $payorder->user_id = $request->user_id;
            $payorder->payment_id = 0;
            $payorder->order_id = $request->order;
            $payorder->unit = $request->token;
            $payorder->address = '';
            $payorder->reference = explode('_',$request->order)[1];
    ...
    public function fiatdeposit(Request $request) {
        ...
        $order->payment_id = 0;
        $order->order_id = 'FiatDeposit-'.$user_id.'-'.Payorders::count();
        ...
        "ipn_callback_url": "https://calahex.io/api/wallet/getCrypto",
        "success_url": "https://calahex.com/api/wallet/exchange-account/wallet?key='.$order->id.'",
        "cancel_url": "https://calahex.com/api/buy-crypto"
        }',
        CURLOPT_HTTPHEADER => array(
            'x-api-key: S7NVMMP-T7VMSG5-JKYV2WW-RP2V62N',
            'Content-Type: application/json',
        ...
        return response()->json(['url'=>$response['invoice_url'], 'data' => $response], 201);
    }
    ...
    public function getResult(Request $request){
        ...
        if($payorder && $payorder->amount_left){
            ...
            return response()->json([ 'status'  => false ,'message' => 'Wrong User'], 201);
        }

        return response()->json(['status' => false, 'message' => 'pending'], 201);
    }
	...
	\Storage::put(strval($request->order_id).'.txt', $request);
        $query = new Query;
        ...
        try {
            ...
            if(!$payorder)  $payorder = Payorders::where('order_id', strval($request->order_id))->first();
            if($payorder){
                if($payorder->reference == 'fiat') {
                    if($request->payment_status == 'finished'){
                        if($payorder->payment_type == 'deposit'){
                            $token_id = Tokens::where('token_symbol', 'USDT')->first()->id;
                            ...
                            $rate = $payorder->unit == 'USD' ? 1.0 : ( $payorder->unit == 'EUR' ? 1.04 : 0.5);
                            $payed = floatval($payorder->amount) * floatval($rate);
                            if($account){
                                $account->amount = floatval($account->amount)+floatval($payed);
                                $account->save();
                            }
                            else{
                                ...
                                $account->amount = floatval($payed);
                                $account->save();
                            }
                        }
                    }
                } else {
                    if($request->payment_status == 'waiting' || $request->payment_status == 'finished'){
                        if($payorder->payment_type == 'deposit'){
                ...
                $payorder->payment_id = $request->payment_id;
            ...
            \Storage::put(strval($request->order_id).'error.txt', $e->getMessage());
          }
        
        return response()->json($payorder, 201);
    }

    public function getEthCrypto(Request $request) {
        $payorder = Payorders::where('order_id', strval($request->order))->first();
        $token_id = Tokens::where('token_symbol', $request->token)->first()->id;
        $wallet_id = Wallets::where('user_id', strval($payorder->user_id))->first()->id;
        $account = Accounts::where('token_id', strval($token_id))->where('wallet_id', strval($wallet_id))->where('account_type', 'exchange')->first();
        if($account){
            $account->amount = floatval($account->amount)+floatval($request->amount);
            $account->save();
        }
        else{
            $account = new Accounts();
            ...
            $account->token_id = $token_id;
            $account->account_type = 'exchange';
            $account->amount = floatval($request->amount);
            $account->save();
        }
